...
import { z } from "zod"
import type { form, input, validation } from "../../Types/type"
 ...
 * @param validations - Validations générales à appliquer au formulaire
...
const generateFormSchema = (formInputs: form["inputs"]) => {
  ...
  const finalShemaField = applyGeneralValidations(formInputs, schemaFields)

  //Verifie si le schéma contient deux champs avec le même nom
...
/**
 * Applique les validations générales aux formulaires
 */
const applyGeneralValidations = (
  formInputs: form["inputs"],
  schemaFields: SchemaFields
) => {
  const schemaFieldsWithConfirmation = applyConfirmationValidation(
    formInputs,
    schemaFields
  )

  return schemaFieldsWithConfirmation
}

/**
 *  Cherche les champs qui ont une confirmation
 */
const findFieldsWithConfirmation = (formInputs: form["inputs"]) => {
  return formInputs.filter((input) => input.confirm === true)
}

/**
 * Applique la validation de confirmation aux champs du formulaire
 * @param formInouts - Configuration des champs du formulaire
 * @param schemaFields - Schéma des champs du formulaire
 * @return Schéma Zod avec validation de confirmation
 */
const applyConfirmationValidation = (
  formInputs: input[],
  schemaFields: SchemaFields
): z.ZodType<any> => {
  const fieldsWithConfirmation = findFieldsWithConfirmation(formInputs)

  if (fieldsWithConfirmation.length === 0) {
    return z.object(schemaFields)
  }

  // Créer tous les champs de confirmation
  const confirmationFields = createConfirmationFields(fieldsWithConfirmation)

  // Combiner les champs originaux et de confirmation
  const allFields = { ...schemaFields, ...confirmationFields }

  // Créer le schéma avec validation de confirmation
  return z.object(allFields).superRefine((data, ctx) => {
    validateConfirmationFields(fieldsWithConfirmation, data, ctx)
  })
}
 ...
 */
const validateConfirmationFields = (
  fieldsWithConfirmation: input[],
  formData: any,
  validationContext: z.RefinementCtx
) => {
  fieldsWithConfirmation.forEach((inputConfig) => {
    const confirmationFieldName = `${inputConfig.name}Confirm`
    const fieldValue = formData[inputConfig.name]
    const confirmationValue = formData[confirmationFieldName]

    if (fieldValue !== confirmationValue) {
      validationContext.addIssue({
        code: z.ZodIssueCode.custom,
        message: `${inputConfig.label} et sa confirmation ne correspondent pas`,
...
/**
 * Crée les champs de confirmation
 */
const createConfirmationFields = (
  fieldsWithConfirmation: input[]
): SchemaFields => {
  const confirmationFields: SchemaFields = {}

  fieldsWithConfirmation.forEach((inputConfig) => {
    const confirmationFieldName = `${inputConfig.name}Confirm`
    confirmationFields[confirmationFieldName] = z.string({
      required_error: `La confirmation de ${inputConfig.label} est requise`,
    })
  })
